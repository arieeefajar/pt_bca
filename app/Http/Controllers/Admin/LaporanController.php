...
class LaporanController extends Controller
{
	...
	public function getPertanyaanKepuasan($category, $area)
	{
		...
		try {
			...
			if ($category == 'product') {
				...
				$variety_previlege5 = isset($variety_previlege['5']) ? $variety_previlege['5'] : 0;
				$variety_previlege4 = isset($variety_previlege['4']) ? $variety_previlege['4'] : 0;
				$variety_previlege3 = isset($variety_previlege['3']) ? $variety_previlege['3'] : 0;
				$variety_previlege['kepuasan'] = number_format((($variety_previlege5 + $variety_previlege4 + $variety_previlege3) / $dataTotalAllKategory) * 100, 2, '.', '');
	...
	public function getPertanyaanKepuasanAll($category)
	{
		...
		try {
			...
			// set customer data berdasarkan daerah sesuai parameter
			foreach ($dataAI['customer_data'] as $value) {
				foreach ($value['answer'] as $value2) {
					array_push($dataAnswer, $value2);
				}
			}
	...
	public function getPertanyaanKepuasanByRespondentsAll($category)
	{
		$endPointApi = env('PYTHON_END_POINT') . 'ai';
		$dataAnswerRaw = [];
		$dataAnswerLoop = [];
		...
		try {
			...
			foreach ($dataAI['customer_data'] as $value) {
				foreach ($value['answer'] as $value2) {
					array_push($dataAnswerRaw, $value2);
				}
			}
	...
	public function getPertanyaanAnalisisPesaingAll($category = 'perusahaan')
	{
		...
		try {
			...
			switch ($category) {
				case "perusahaan":
					$allowedKeys = $allowedKeysPerusahaan;
					break;
				case "pendatang_baru":
					$allowedKeys = $allowedKeysPendatangBaru;
					break;
				case "substitusi":
					$allowedKeys = $allowedKeysSubstitusi;
					break;
				case "pemasok":
					$allowedKeys = $allowedKeysMenawarPemasok;
					break;
				case "pembeli":
					$allowedKeys = $allowedKeysMenawarPembeli;
					break;
				default:
			}
	...
	public function getPertanyaanAnalisisPesaingByRespondentsAll($category = 'perusahaan')
	{
		$endPointApi = env('PYTHON_END_POINT') . 'competitor-analys';
	...
	public function getPertanyaanKekuatanKelemahanByRespondentsAll($category)
	{
		$endPointApi = env('PYTHON_END_POINT') . 'ai';
		$dataAnswerRaw = [];
		$dataAnswerLoop = [];
